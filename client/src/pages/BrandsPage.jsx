...
const brandGroups = [
  { name: 'Electrical', items: brandsData.slice(0, 5) },
  { name: 'Paint', items: brandsData.slice(5, 9) },
  { name: 'Plumbing & Sanitary Ware', items: brandsData.slice(9, 14) },
  { name: 'Locks & Hardware', items: brandsData.slice(14, 19) },
  { name: 'Safety, Tools & Industrial', items: brandsData.slice(19) },
];
    ...
    <>
      ...
      <section className="brands-section">
        <div className="brand-intro">
          <strong>Brands We Deal With</strong>
          <span>Organised by product category so you can find the right source quickly.</span>
        </div>
        {brandGroups.map(group => <div className="logo-group" key={group.name}>
          <h2 className="marquee-title">{group.name}</h2>
          <div className="logo-wall">
            <div className="logo-grid">
              {group.items.map((item, index) => (
                <div className="logo-card" key={item.id} style={{ '--logo-order': index }}>
                  <img src={item.logo} alt={`${group.name} brand logo`} />
                </div>
              ))}
            </div>
          </div>
        </div>)}
        <div className="brand-signatures"><b>RKA Innovations</b><span>RKA Vellore</span><strong>Vellore Premium Bath Fittings</strong></div>
      </section>
      ...
      <style>{`
        ...
        .brand-intro { display: grid; gap: 8px; max-width: 1280px; margin: 0 auto 36px; text-align: center; }
        .brand-intro strong { font-size: clamp(28px, 4vw, 46px); color: #172125; }
        .brand-intro span { color: #667176; }
        .brand-signatures { display: flex; flex-wrap: wrap; justify-content: center; gap: 14px 28px; padding: 28px 0 8px; color: #172125; }
        .brand-signatures span, .brand-signatures strong { color: #e31b16; }
